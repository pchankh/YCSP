#= samin.jl. Translation of samin.cc code (in the optim package
   for GNU Octave) to Julia

  Copyright (C) 2016   Michael Creel   <michael.creel@uab.es>
...
KEYWORDS
* nt:  integer: # reduce temperature every nt*ns*dim(x_init) evaluations
* ns:  integer: # adjust bounds every ns*dim(x_init) evaluations
* rt:  (0 < rt <1): geometric temperature reduction factor: when temp changes, new temp is t=rt*t 
* maxevals:  integer: limit on function evaluations
* neps:  integer:  number of previous best values the final result is compared to
...
=#
function samin(obj_fn, x, lb, ub; nt=5, ns=5, rt=0.5, maxevals=1e6, neps=5, functol=1e-8, paramtol=1e-5, verbosity=1)
