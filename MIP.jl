...
function MIP(H, N, n, heightsInitial, stackOf, heightOf, loadOf, toBeUnloaded, realStack, IOPoints, SR, SB, SO, SX, SY, anteriorStacks, posteriorStacks, posCraneInitial, T, contMinHeightStack, previousContToMove,  costMove, costPreMove, costToGo, alpha, printSolver, gapOfMIP, limitOfTime)
    ...
    @variable(IPModel, w[1:T,1:T], Bin);
    ...
    for m = 2:N
        @constraint(IPModel, sum(w[m-1,u] for u = 1:t-1) - w[m,t] >= 0);
    end
    ...
    for m = 1:n
        for t = 1:T
            # Relation between variables x and w for retrievals
            @constraint(IPModel, sum(x[stackOf[m],r,t] for r in loadOf[m]) - w[m,t] >= 0);
        end
    end
    ...
    for m = n+1:N
        for t = 1:T
            ...
            @constraint(IPModel, sum(x[s,r,t] for s in unloadFrom[m] for r in posteriorStacks[s]) - w[m,t] >= 0);
        end
    end
    ...
    for m = N+1:T
        for t = 1:T
            ...
            @constraint(IPModel, sum(x[stackOf[m],r,t] for r in intersect(posteriorStacks[stackOf[m]],SB)) - w[m,t] >= 0);
        end
    end
