...
(T,SR,SO,SL,SY,stackOf,heightOf,loadOf,contStackIOPoint,contMinHeightStack,previousContToMove,toRetrieve,toBeLoaded, deadlineRet) = initializeRetrievals(randomRetrieval,n,S,SB,heightsInitial,IOPoints, groupIOPoint,IOPointsPosition);

(unloadFrom, SU, SX, contStackableIOPoint, toBeUnloaded, deadlineSta) = initializeStackings(N,n,randomStacking,IOPointsPosition,SR,groupIOPoint);
...
deadlines = defineRandomDeadlines(deadlineRet, deadlineSta, randomRetrieval, randomStacking, N, n, T, costMove, costPreMove);

printProblem(R,S,H,IOPointsPosition,N,n,toRetrieve,toBeLoaded,toBeUnloaded,stackCost,rowCost,relocCost,costToGo,heightsInitial,posCraneInitial,deadlines);
...
(X,DInit,D,finalHeights,ServTimes,W,obj,timeToSolve) = MIP(H, N, n, artificialHeights, moveFrom, toBeUnloaded, IOPoints, SR, SB, SO, SX, SY, anteriorStacks, posteriorStacks, posCraneInitial, T, contMinHeightStack, previousContToMove,  costMove, costPreMove, costToGo, alpha, deadlines, printSolver, gapOfMIP, limitOfTime);
