...
function BIP(limitOfTime,T,moveFrom,SX,posteriorStacks,SY,SB,Z,costMove,costPreMove,posCraneInitial,costToGo,alpha,N,realToClusterOrder,changeOfOrder,typeOfTruck,previousContToMove,posteriorContStacks,SR,contMinHeightStack,anteriorStacks,artificialHeights)
    ...
    @constraint(IPModel, orderConstraint_1[o = 1:N,t = 1:T], sum(w[realToClusterOrder[p],s,u] for p = 1:N for s in moveFrom[realToClusterOrder[p]] for u = 1:t-1) + (t - 1 - (o+changeOfOrder[typeOfTruck[o]]-1)) * sum(w[realToClusterOrder[o],s,t] for s in moveFrom[realToClusterOrder[o]]) <= t-1);
    @constraint(IPModel, orderConstraint_2[o = 1:N,t = 1:T], sum(w[realToClusterOrder[p],s,u] for p = 1:N for s in moveFrom[realToClusterOrder[p]] for u = t+1:T) + (T - t - (N-o+changeOfOrder[typeOfTruck[o]])) * sum(w[realToClusterOrder[o],s,t] for s in moveFrom[realToClusterOrder[o]]) <= T - t);
